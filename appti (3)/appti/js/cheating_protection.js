...
const CheatingProtection = (() => {
    ...
    function isEditableElement(el) {
        ...
        return tag === 'textarea' || tag === 'input' || el.isContentEditable || 
               ...
               el.classList.contains('scratchpad-textarea');
    }
    ...
    function triggerScreenshotBlockShield(reason = "Screenshot attempt intercepted") {
        ...
        if (!shield) {
            ...
            shield.style.cssText = `
                ...
                background: rgba(3, 7, 18, 0.96); z-index: 999999;
                display: flex; align-items: center; justify-content: center;
                backdrop-filter: blur(25px); padding: 1.5rem; box-sizing: border-box;
                animation: fadeIn 0.2s ease;
            `;

            shield.innerHTML = `
                <div class="glass-panel" style="width: 100%; max-width: 480px; padding: 2.5rem 2rem; border-radius: var(--radius-lg); text-align: center; border: 1px solid var(--danger); box-shadow: 0 0 50px rgba(244, 63, 94, 0.3); background: radial-gradient(circle at top, rgba(244, 63, 94, 0.15) 0%, rgba(8, 12, 24, 0.98) 70%);">
                    ...
                    <h3 style="font-size: 1.45rem; font-family: var(--font-heading); margin: 0 0 0.5rem; color: #ffffff;">SCREENSHOT RESTRICTED</h3>
                    <p style="color: var(--text-secondary); font-size: 0.92rem; line-height: 1.6; margin-bottom: 1.8rem;">
                        Screenshotting and screen recording of proprietary assessment materials are strictly prohibited to maintain academic integrity.
                    </p>
        ...
        // Clear clipboard immediately so copied image data is destroyed
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText('').catch(() => {});
        }

        dispatchToast("🚨 Screenshot attempt blocked by security engine.", "danger");
    }

    // --- 1. Global Content & Anti-Screenshot Protection ---
    function initGlobalProtection() {
        ...
        // Block Right-Click Context Menu (except in coding editor / inputs)
        ...
        // Block Copy & Cut on question statements and options
        document.addEventListener('copy', (e) => {
            if (isEditableElement(e.target)) return true;
            e.preventDefault();
            if (e.clipboardData) e.clipboardData.setData('text/plain', '');
            dispatchToast("⚠️ Question copying is disabled.", "warning");
            return false;
        }, options);
        ...
        // Keydown Shortcut Interceptor (PrintScreen, Win+Shift+S, F12, DevTools, Copy)
        window.addEventListener('keyup', (e) => {
            // PrintScreen keyup detection
        ...
        window.addEventListener('keydown', (e) => {
            ...
            // View Source (Ctrl+U) or Save (Ctrl+S) or Print (Ctrl+P)
        ...
        // Visibility Change / Tab Switching Detection during Active Tests
        ...
        console.log("[CheatingProtection]: Comprehensive Anti-Screenshot & Exam Proctoring active.");
    }
    ...
    function showSecurityConsent(onAccept) {
        ...
        modal.style.cssText = `
            ...
            display: flex; align-items: center; justify-content: center;
            backdrop-filter: blur(14px); padding: 1.5rem; box-sizing: border-box;
            ...
            <div class="glass-panel" style="width: 100%; max-width: 480px; border-radius: var(--radius-lg); padding: 2.2rem; background: radial-gradient(circle at top right, rgba(0, 240, 255, 0.12) 0%, rgba(8, 14, 28, 0.98) 75%); border: 1px solid var(--border-highlight); box-shadow: 0 30px 70px rgba(0,0,0,0.8); text-align: center; display: flex; flex-direction: column; gap: 1.2rem;">
                ...
                <div style="background: rgba(255,255,255,0.03); border: 1px solid var(--glass-border); border-radius: var(--radius-sm); padding: 1rem; text-align: left; font-size: 0.85rem; color: var(--text-secondary); display: flex; flex-direction: column; gap: 0.6rem;">
                    <div style="display: flex; align-items: center; gap: 0.6rem;">
                        <i class="fa-solid fa-camera-slash" style="color: var(--danger);"></i> <span>Screenshotting and screen recording are prohibited</span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 0.6rem;">
