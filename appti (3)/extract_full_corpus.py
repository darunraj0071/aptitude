...
def extract_chapter_questions_advanced(ch_paras, ch_title=""):
    # 1. Separate into Questions part and Solutions part
    sol_idx = -1
    ...
    for sec_name, paras in sec_q_paras.items():
        ...
        while i < len(paras):
            ...
            if re.search(r'Directions?\s*(\(Qs?\.?|\(Question|\d+)', line, re.I):
                ...
                # Check if direction is followed by "Give answer:" or option definitions
                dir_opt_lines = []
                while i < len(paras):
                    cand = paras[i].strip()
            ...
            while i < len(paras):
                ...
                if re.search(r'Directions?\s*\(', next_l, re.I):
                    break
                if line.startswith("Statements:") and next_l.startswith("Statements:"):
                    break
            ...
            if not opts_dict:
                # Use direction options if available
